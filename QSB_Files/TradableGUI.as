package QSB_Files
{
	...
	public class TradableGUI extends MovieClip
	{
		...
		function SetGUI():void
		{
			...
			TwClip.graphics.drawRoundRect(-600, 100, 350, 375, 12);
			TwClip.filters = [dsF];
			//
			...
			tween = new Tween(TwClip,"x",Bounce.easeOut,TwClip.x,600,1,true);
			tween.start();
		}
		...
		function SetTreatments(e:Event):void
		{
			...
			_Tradable.STOCK_INDICATOR = autoComplete.text;
		...
		function SetStkBx():void
		{
			...
			autoComplete.addEventListener(Event.SELECT, StkBxHandler);
			autoComplete.autoFillEnabled = true;
			TwClip.addChild(autoComplete);
		...
		function SetStocks(e:Event):void
		{
			...
			autoComplete.dataProvider = new DataProvider(tempArray); 
		...
		function StkBxHandler(e:Event):void{
			...
			trace(autoComplete.selectedItem);
			var temp:String = autoComplete.text;
		...
		public function DestorySelf(theStage:MovieClip):void{
